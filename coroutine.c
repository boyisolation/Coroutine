...
int coroutine_create(schedule_t *s,void*(*call_back)(schedule_t *,void *args),void *args)
{
  ...
  makecontext(&c->ctx,(void (*)())&main_fun,1,s);//一个参数，s
...
void coroutine_yield(schedule_t *s)
{
  if(s->current_id!=-1)
  {
    coroutine_t *c=s->coroutines[s->current_id];//在调度器中找到调出的协程
    c->state=SUSPEND;
    s->current_id=-1;
    swapcontext(&c->ctx,&s->ctx_main);//保存当前流程，返回主流程
  }
}
